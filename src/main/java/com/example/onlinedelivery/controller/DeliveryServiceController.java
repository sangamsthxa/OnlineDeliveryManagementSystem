...
public class DeliveryServiceController {
	...
	@Autowired
	private GenericService<DeliveryService> deliveryService;

	@Autowired
	private DeliveryServiceRepository deliveryRepo;

	@PostMapping("/save")
	public ResponseEntity<ResponseMessage> saveDeliveryService(@Valid @RequestBody DeliveryService ds, Errors error) {
		ResponseMessage response = new ResponseMessage();

		ds.setCreatedAt(new Date());
		ds.setUpdatedAt(new Date());
		deliveryService.saveInfo(ds);
	...
	@PutMapping("/update")
	public ResponseEntity<ResponseMessage> updateDeliveryService(@Valid @RequestBody DeliveryService ds, Errors error) {

		ResponseMessage response = new ResponseMessage();

		ds.setCreatedAt(new Date());
		ds.setUpdatedAt(new Date());
		deliveryService.updateInfo(ds);
		response.setErrors(null);
		response.setMessage("Success");
		response.setStatus(true);
		response.setStatusCode(HttpStatus.OK.value());
	...
	public ResponseEntity<ResponseMessage> deleteById(@PathVariable("id") int id) {
		...
		boolean status = deliveryRepo.existsById(id);
		if (!status) {
	...
	public List<DeliveryService> getAllDeliveryServiceInfo() {
		List<DeliveryService> ds = deliveryRepo.findAll();
		if (ds.isEmpty()) {
			throw new RuntimeException("Delivery Service List not Exist");
		}
		return ds;
	}
